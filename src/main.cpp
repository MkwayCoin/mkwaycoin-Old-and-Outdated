...
#if defined(NDEBUG)
# error "Mkwaycoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x56450e063d296a82f3bb34f8902c5d365306fb03aa601f51f270824a4db25a38");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Mkwaycoin: starting difficulty is 1 / 2^12
...
{
    // Mkwaycoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 1000000); // Mkwaycoin: 1000k blocks in ~2 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 1 * 24 * 60 * 60; // Mkwaycoin: 3.5 days
static const int64 nTargetSpacing = 1 * 60; // Mkwaycoin: 1.0 minutes
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Mkwaycoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool SetBestChain(CValidationState &state, CBlockIndex* pindexNew)
{
    ...
    if ((pindexNew->nHeight % 20190) == 0 || (!fIsInitialDownload && (pindexNew->nHeight % 144) == 0))
    {
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Mkwaycoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x3583901686d443c010d635e194ecbfd426dc3dc944d9a2e57ed3bc42055884d6");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "Ryan Whitwam on June 27, 2019 Scientists Simulate Human Colonization of the Milky Way";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        txNew.vin[0].scriptSig = CScript() << 486604799 << CBigNum(4) << vector<unsigned char>((const unsigned char*)pszTimestamp, (const unsigned char*)pszTimestamp + strlen(pszTimestamp));
        txNew.vout[0].nValue = 100 * COIN;
        txNew.vout[0].scriptPubKey = CScript() << ParseHex("0463fa9a2710f1dbcdca69ebf15e127991bbffa04cf13e0ac5f096d513aff46859d4ce02cfe6eab73f1767b2c1ad6f05d1f739e1822942eeec692c477944e6f55e") << OP_CHECKSIG;
        CBlock block;
        block.vtx.push_back(txNew);
        block.hashPrevBlock = 0;
        block.hashMerkleRoot = block.BuildMerkleTree();
        block.nVersion = 1;
        block.nTime    = 1576241225;
        block.nBits    = 0x1e0ffff0;
        block.nNonce   = 2085174839;

        if (fTestNet)
        {
            block.nTime    = 1576241208;
            block.nNonce   = 387379568;
        }
...
if (false && block.GetHash() != hashGenesisBlock)
        ...
        assert(block.hashMerkleRoot == uint256("0x1995a0783e112582730cca277b628acd2a1937add9a1e2af520d6d39c123ccee"));
        block.print();
...
unsigned char pchMessageStart[4] = { 0xb5, 0xfe, 0x6f, 0xd7 }; // Mkwaycoin: increase each by adding 2 to bitcoin's value.
...
void static MkwaycoinMiner(CWallet *pwallet)
{
    printf("MkwaycoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("mkwaycoin-miner");
    ...
    {
        printf("MkwaycoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&MkwaycoinMiner, pwallet));
}
